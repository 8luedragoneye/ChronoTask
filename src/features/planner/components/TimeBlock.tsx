...
import type { TaskType as TaskTypeEntity } from '../../../core/entities/TaskType'
...
export function TimeBlock({ task, workStartHour, pixelsPerMinute, onDelete }: TimeBlockProps) {
  ...
  const [taskTypeEntity, setTaskTypeEntity] = useState<TaskTypeEntity | null>(null)
  ...
  // Use color from task type entity or default (keep text black for better readability)
  const defaultColor = '#3B82F6' // Blue default
  const taskTypeColor = taskTypeEntity ? {
    backgroundColor: `${taskTypeEntity.color}20`,
    borderColor: taskTypeEntity.color,
    color: '#000000', // Always black text for better readability
  } : {
  ...
  return (
    <div
      ...
      className={`border-l-4 rounded-r px-2 py-1 text-sm shadow-sm hover:shadow-md transition-shadow ${isDragging ? 'opacity-50 z-50' : ''}`}
